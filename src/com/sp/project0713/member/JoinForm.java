/*
JDBC - 자바언어로 데이터베이스를 제어하는 기술을 가리킴
Java Database Connectivity
java.sql패키지에서 지원함
*/
package com.sp.project0713.member;
...
class JoinForm extends JFrame implements ActionListener{
	...
	JTextField t_name;
	JTextField t_phone;
	JButton bt_connect;//데이터베이스 접속버튼
	...
	//Connection객체란? 접속 성공을 하면 그 접속정보를 보유한객체
	Connection con=null;
	...
	public void connect(){
		...
		con=DriverManager.getConnection(url2,"java","1234");
			...
			System.out.println("드라이버가 존재하지않습니다");
	...
	public void regist(){
		...
		String phone=t_phone.getText();
					...
					//준비된 쿼리문을 실행하기
					//executeUpdate()메서드는 DML을 수행할 수있는데 이때 수행결과
					//로 이 쿼리실행에 의해 영향을 받은 레코드 수를 반환한다. 따라서 개발자는 
					//그 결과가 0이면 DML수행이 안되었다는 것을 알수있다..
	...
	public void registOracle(){
		...
		if(result>0){
			JOptionPane.showMessageDialog(this,"오라클등록성공");
		}
		}catch(SQLException e){
			e.printStackTrace();
		}
